...
data_taxi<- data[data$car_type_cat=="Taxi",]
data_bus<- data[data$car_type_cat=="Bus",]

#Ra for taxi and bus separate.......................................
for (i in nrow(data_taxi)) { 
      data_taxi<- data_taxi %>% 
      mutate(Ra = ifelse(
        (v>=6) & (Diff_v>=8),"Yes", "No"))
  }

for (i in nrow(data_bus)) { 
  data_bus<- data_bus %>% 
    mutate(Ra = ifelse(
      (v>=6) & (Diff_v>=6),"Yes", "No"))
}
...
for (i in nrow(data_taxi)){
    ...
    data_taxi<- data_taxi %>% 
      mutate(Rd = ifelse((Diff_v<=-14) & (v>=6),"Yes", "No"))
}

for (i in nrow(data_bus)) {
    
    data_bus<- data_bus %>% 
      mutate(Rd = ifelse((Diff_v<=-9) &(v>=6),"Yes", "No"))
}
...
for (i in nrow(data_taxi)) {
    ...
    data_taxi<- data_taxi %>% 
      mutate(Rstop = ifelse((Diff_v<=-14) & (v<=5),"Yes", "No"))
}

for (i in nrow(data_bus)){
    
    data_bus<- data_bus %>% 
      mutate(Rstop = ifelse((Diff_v<=-9) & (v<=5), "Yes", "No"))
  }  
...
write.csv(data_taxi,"D:/BILAL/output_taxi.csv", row.names = TRUE)
write.csv(data_bus,"D:/BILAL/output_bus.csv", row.names = TRUE)
